...
SELECT
    ...
    user_id,
    SUM(amount_spent) AS total_purchase
FROM
    purchases
WHERE
    EXTRACT(MONTH FROM date_of_purchase) = 1
GROUP BY
    month,user_id
HAVING
...
WITH CTE AS (
    SELECT
        ...
        user_id,
        total_purchase,
        LAG(total_purchase) OVER (PARTITION BY user_id ORDER BY month) AS previous_month_total
    FROM (
        SELECT
            EXTRACT(MONTH FROM date_of_purchase) AS month,
            user_id,
...
SELECT
    department,
    ROUND(AVG(CASE WHEN end_date ISNULL THEN CURRENT_DATE - start_date
    ELSE end_date - start_date END),2) AS average_duration
FROM
    employee_service
GROUP BY
    department;