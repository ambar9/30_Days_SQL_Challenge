...
FROM
    sales1
WHERE
    EXTRACT(MONTH FROM saledate) = 2 --EXTRACT(MONTH FROM CURRENT_DATE - INTERVAL '1 month')
    AND
    EXTRACT(YEAR FROM saledate) = EXTRACT(YEAR FROM CURRENT_DATE - INTERVAL '1 MONTH')
GROUP BY
    region
ORDER BY
    total_sale
